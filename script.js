...
function initCareersForm() {
  ...
  $('#careerForm').addEventListener('submit', async e => {
    ...
    if (app.phone.replace(/\D/g, '').length < 10) { toast('Please enter a valid 10-digit phone number.'); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(app.email)) { toast('Please enter a valid email address.'); return; }
    ...
    const result = await MockEmail.send({ to: 'careers', subject, body, leadData: app });
    ...
    if (result.ok) {
      ...
      toast('Could not submit. Please try again or WhatsApp us.');
    }
  });
}
